...
from models.physical_interaction import PhysicalInteractionEncoder
...
class ContextNodeEdgeNet(Module):
    def __init__(self, node_dim, edge_dim, hidden_dim,
        ...
        # Physical interaction encoder (optional, for enhanced physical modeling)
        self.use_physical_interactions = kwargs.get('use_physical_interactions', False)
        if self.use_physical_interactions and not node_only:
            physical_hidden_dim = kwargs.get('physical_hidden_dim', hidden_dim)
            self.physical_encoder = PhysicalInteractionEncoder(
                node_dim=node_dim,
                edge_dim=edge_dim,
                hidden_dim=physical_hidden_dim,
                use_learnable_params=kwargs.get('physical_learnable', True)
            )
            # Adjust input edge dimension to include physical features
            input_edge_dim += physical_hidden_dim
        else:
    ...
    def forward(self, h_node, pos_node, h_edge, edge_index,
        ...
        context node features
            ...
            pos_ctx: (n_ctx, 3)
            batch_ctx: (n_ctx, )
        atom_types: (n_node,) Optional atom type indices for physical interaction encoding
        Output:
            h_node: (n_node, node_dim)
            h_edge: (n_edge, edge_dim)
            pos_node: (n_node, 3)
        ...
        for i in range(self.num_blocks):
            ...
            # Add physical interaction features if enabled
            if self.use_physical_interactions and self.physical_encoder is not None and not self.node_only:
                if atom_types is not None:
                    # Use provided atom types
                    physical_feat = self.physical_encoder(
                        h_node, edge_index, distance, atom_types, h_edge=h_edge
                    )
                    h_edge = torch.cat([h_edge, h_dist, physical_feat], dim=-1)
                else:
                    # Fallback: use distance-based features only
                    # In practice, atom_types should be provided for best results
