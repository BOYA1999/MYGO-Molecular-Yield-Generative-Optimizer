...
class ForeverTaskDataset(IterableDataset):
    ...
    def setup(self, set_index=False, set_db=False, set_iter=False,
        ...
        if set_iter:
            ...
            self.total_samplers = total_samplers = max(1, self.num_workers * self.world_size)  # Ensure at least 1
    ...
    def _get_global_id(self):
        ...
        if self.num_workers == 0:
            num_workers = 1
            worker_id = 0
        assert num_workers == max(1, self.num_workers)
        global_id = self.gpu_rank * max(1, self.num_workers) + worker_id
    ...
    def get_data_key(self, task, db_name, data_id):
        ...
        elif db_name in ['csd', 'pbdock', 'pbdockS', 'pbdockL', 'moad', 'data_mine']:
