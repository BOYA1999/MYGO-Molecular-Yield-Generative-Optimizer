...
def time_limit(seconds):
    """
    Cross-platform time limit context manager.

    - On Unix-like OSes, uses SIGALRM.
    - On Windows (no SIGALRM), this becomes a no-op. Use `run_with_timeout`
      for operations that must be bounded on Windows.
    """
...
def run_with_timeout(fn, timeout_s: int, *args, **kwargs):
    """
    Run a callable with a timeout in seconds.

    This is the preferred mechanism for enforcing timeouts on Windows.
    """
    with ThreadPoolExecutor(max_workers=1) as ex:
        ...
        try:
            ...
            raise TimeoutException("Timed out!") from e
