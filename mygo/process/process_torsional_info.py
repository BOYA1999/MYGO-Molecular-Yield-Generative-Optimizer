...
# from process.unmi.process_mols import get_unmi_raw_db, unmi_data_to_rdmol  # Removed for SBDD-only version
...
def get_mol_from_data(data_dict, mols_dir=None, train_txn=None, val_txn=None, root_dir=None):
    if mols_dir is None:
        ...
        elif db in ['moad', 'pbdock', 'csd', 'apep', 'pepbdb', 'poseb', 'poseboff', 'data_mine']:
    ...
    # Check if this is from data_mine based on mols_dir path
    if 'data_mine' in mols_dir:
        ...
        if db_name in ['geom', 'qm9', 'cremp']:
            ...
            # UNMI support removed for SBDD-only version
            raise NotImplementedError('UNMI database support removed for SBDD-only version')
        else:
            # Default to _mol.sdf for unknown patterns
            mol_fn = data_id + '_mol.sdf'
...
def get_torsional_info_mol(mol, bond_index, data_id=None):
    ...
    for rot_bond in rotatable_bond_index.T:
        ...
        if len(connected_components) == 2:
            ...
            # Skip this rotatable bond if it doesn't create two components
            continue
...
def get_db_config(db_name, save_name='torsion', root='data/data_train'):
    ...
    elif db_name == 'data_mine':  # with pocket
        mols_dir = os.path.join(data_dir, 'files/mols')
        mol_path = os.path.join(data_dir, 'lmdb/pocmol10.lmdb')
