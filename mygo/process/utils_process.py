...
# Peptide imports removed for SBDD-only version
...
from utils.graph_decom import decompose_brics, decompose_mmpa
...
# build_peptide function removed for SBDD-only version


# add_pep_bb_data function removed for SBDD-only version
...
# get_peptide_info function removed for SBDD-only version
...
def get_pocmol_data(mol, pdb, data_id='', pdbid='', return_mol=False):
    ...
    if isinstance(mol, str):
        ...
        # Peptide handling removed for SBDD-only version
        else: # smiles
...
# process_raw_pep function removed for SBDD-only version